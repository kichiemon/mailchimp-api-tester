...
import { CreateCampaignInput, UpdateCampaignInput, CreateSegmentInput, UpdateSegmentInput } from '@/types/mailchimp'
...
export async function createCampaign(campaign: CreateCampaignInput): Promise<any> {
  return fetchAPI('POST', '/campaigns', campaign)
}

export async function updateCampaign(id: string, updates: UpdateCampaignInput): Promise<any> {
...
export async function getSegment(listId: string, segmentId: string): Promise<any> {
  return fetchAPI('GET', `/lists/${listId}/segments/${segmentId}`)
}

export async function createSegment(listId: string, segment: CreateSegmentInput): Promise<any> {
  return fetchAPI('POST', `/lists/${listId}/segments`, segment)
}

export async function updateSegment(listId: string, segmentId: string, updates: UpdateSegmentInput): Promise<any> {
  return fetchAPI('PATCH', `/lists/${listId}/segments/${segmentId}`, updates)
}

export async function deleteSegment(listId: string, segmentId: string): Promise<any> {
